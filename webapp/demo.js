...
const _dev = {
  getprop: async (k) => ({ "ro.product.model":"DongfengAeolus","ro.product.device":"spm8675p1_64_raite","ro.build.version.release":"11" }[k] || ""),
  shell: async () => ({ stdout: "0" }), setprop: async () => true,
  install: async () => { await new Promise(r=>setTimeout(r,380)); return { ok:true }; },
  close: async () => {},
};
async function transport() { return { connectUsb: async () => _dev, connectRelay: async () => _dev }; }
...
async function api(path) {
  await sleep(400);
  if (path === "/api/request") return { rid:"demo", code:"K7Q3M9" };
  if (path.startsWith("/api/status")) return (++_p >= 3) ? { status:"approved", token:"demo" } : { status:"pending" };
  if (path === "/api/plan") return { profile:"dongfeng_mage", name:"Dongfeng Aeolus MAGE", ops:[
  {op:"root"},{op:"setprop",key:"persist.apk.sign.verify",value:"0"},
  {op:"install",file:"df01_freetube.apk",flags:["-r","-g"],sha256:"222a15442da6e36676cb6f5a2bcfc4589be4cfe940ce5079bc6c9633a7620f20",bytes:4.5e6},
  {op:"install",file:"df02_yandexnavi.apk",flags:["-r","-g"],sha256:"6e0260b07e1f909e9dea2b2896932b4a6dd4cf1dc277abfe387963e7f6dafe2f",bytes:106e6},
  {op:"shell",cmd:"pm grant ru.yandex.yandexnavi android.permission.ACCESS_FINE_LOCATION"},
  {op:"sqlite",db:"launcher.db",stmt:"delete …"},{op:"sqlite",db:"launcher.db",stmt:"insert …"},
  {op:"sqlite",db:"launcher.db",stmt:"delete …"},{op:"sqlite",db:"launcher.db",stmt:"insert …"},
...
$("btn-again").onclick = () => location.href = location.pathname;
...
async function download(op, i, label) {
  ...
  const steps = op.bytes > 5e7 ? 28 : 13;
  for (let s = 1; s <= steps; s++) { progress(base + span * (s/steps)); await sleep(op.bytes > 5e7 ? 46 : 24); }
  return new ArrayBuffer(8);
}

// --- utils -----------------------------------------------------------------
async function sha256ok(buf, expected) {
  if (new URLSearchParams(location.search).has("fail") && expected.startsWith("6e02")) return false;
  return true;
}
